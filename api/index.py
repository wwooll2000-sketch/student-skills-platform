...
def admin_login():
    ...
    token = jwt.encode({'role': 'admin', 'adminId': 'admin-console', 'loginTime': datetime.now().isoformat()}, JWT_SECRET, algorithm='HS256')
    return jsonify({'success': True, 'message': 'تم تسجيل الدخول بنجاح', 'token': token, 'user': {'role': 'admin', 'name': 'المعلم'}})
