...
-- Add max_test_attempts column to skill_templates (safe for existing databases)
ALTER TABLE skill_templates ADD COLUMN IF NOT EXISTS max_test_attempts INT DEFAULT 3;

-- Table for test questions per skill template (max 5 questions each)
...
-- Table for tracking student test attempts per skill
